...
void BoxCollisionComponent::GetMyObb(OBB& myObb)
{
	...
	// OBBの中心を調整 / モデルの原点がずれている場合、回転すると中心もずれるため
	...
	myObb.s_Axis[0] = XMLoadFloat3(&axisX);
	myObb.s_Axis[1] = XMLoadFloat3(&axisY);
	myObb.s_Axis[2] = XMLoadFloat3(&axisZ);

	// OBBに値を設定
	myObb.s_Center = myCenter;
	myObb.s_Size = mySize;
}
...
bool BoxCollisionComponent::CheckHitObject(const OBJECT& object)
{
	...
	for (GameObject* object : m_GameObjectsCache[static_cast<int>(object)])
	{
		...
		for (BoxCollisionComponent* boxCollision : boxCollisions)
		{
			...
			boxObb.s_Axis[0] = XMLoadFloat3(&boxAxisX);
			boxObb.s_Axis[1] = XMLoadFloat3(&boxAxisY);
			boxObb.s_Axis[2] = XMLoadFloat3(&boxAxisZ);

			// OBBに値を設定
			boxObb.s_Center = boxCenter;
			boxObb.s_Size = boxSize;
